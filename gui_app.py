...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.stop_fuzz_activities = False
        ...
        self.sidebar_frame = customtkinter.CTkScrollableFrame(self, width=180, corner_radius=0,
        ...
        self.scaling_optionemenu.grid(row=52, column=0, padx=20, pady=(5, 40))
        ...
        self.consoles_tabview.add("IPC Traffic")

        self.ipc_textbox = customtkinter.CTkTextbox(self.consoles_tabview.tab("IPC Traffic"), state="disabled",
                                                        border_width=2, font=("consolas", 15), wrap='none')
        self.ipc_textbox.grid(row=0, column=0, columnspan=100,rowspan=100, padx=(0, 0), pady=(0, 5), sticky="nsew")
        self.consoles_tabview.tab("IPC Traffic").grid_columnconfigure(0,weight=1)
        self.consoles_tabview.tab("IPC Traffic").grid_rowconfigure(0,weight=1)
        self.print_to_ipc_textBox(text="IPC Traffic",color='green')
        ...
        self.file_explorer_label = customtkinter.CTkLabel(self.consoles_tabview.tab("File Explorer"), text='Path:',
                                                          font=customtkinter.CTkFont(size=15))
        self.file_explorer_label.grid(row=0, column=0, padx=0, pady=(5, 5), sticky="w")
        self.file_explorer_entry = customtkinter.CTkEntry(self.consoles_tabview.tab("File Explorer"),
                                                          placeholder_text="Path",
                                                          font=("consolas", 15))

        self.file_explorer_entry.grid(row=1, column=0, columnspan=100, padx=(0, 0), pady=10, sticky="nsew")
        self.file_explorer_entry.bind("<KeyPress>", self.file_explorer_serach_key_pressed)

        self.upload_file_explorer_button = customtkinter.CTkButton(master=self.consoles_tabview.tab("File Explorer"),
                                                                   fg_color="transparent", text="Upload file",
                                                                   command=self.upload_file_explorer, border_width=2,
                                                                   text_color=("gray10", "#DCE4EE"))
        self.upload_file_explorer_button.grid(row=2, column=0, padx=5, pady=0,sticky='ew')

        self.create_folder_file_explorer_button = customtkinter.CTkButton(master=self.consoles_tabview.tab("File Explorer"),
                                                                   fg_color="transparent", text="Create Folder",
                                                                   command=self.create_folder_file_explorer, border_width=2,
                                                                   text_color=("gray10", "#DCE4EE"))
        self.create_folder_file_explorer_button.grid(row=2, column=1, padx=5, pady=0,sticky='ew')
        ...
        self.scrollable_file_explorer = customtkinter.CTkScrollableFrame(self.consoles_tabview.tab("File Explorer"),
                                                                         ...
                                                                         scrollbar_button_color='#333333')
        self.scrollable_file_explorer.grid(row=3, rowspan=100, column=0,columnspan=100, padx=(5, 5), pady=(5, 5), sticky="nsew")
        ...
        self.consoles_tabview.tab("File Explorer").grid_columnconfigure(1, weight=1)
        self.consoles_tabview.tab("File Explorer").grid_rowconfigure(2, weight=0)
        self.consoles_tabview.tab("File Explorer").grid_rowconfigure(3, weight=1)
        ...
        self.exploit_textbox = customtkinter.CTkTextbox(self.consoles_tabview.tab("Exploit"), state="disabled",
                                                        border_width=2, font=("consolas", 15), wrap='none')
        self.exploit_textbox.grid(row=0, column=0, columnspan=3, padx=(0, 0), pady=(0, 5), sticky="nsew")

        self.exported_activities_menu = customtkinter.CTkOptionMenu(self.consoles_tabview.tab("Exploit"),
        ...
        self.activities_button = customtkinter.CTkButton(self.consoles_tabview.tab("Exploit"), text="Open Activity",
                                                         command=self.open_activity_button_clicked)
        self.activities_button.grid(row=1, column=1, padx=10, pady=(5, 5), sticky="ew")

        self.exported_services_menu = customtkinter.CTkOptionMenu(self.consoles_tabview.tab("Exploit"),
                                                                  dynamic_resizing=True, values=[])
        ...
        self.exported_receivers_menu = customtkinter.CTkOptionMenu(self.consoles_tabview.tab("Exploit"),
                                                                   dynamic_resizing=True, values=[])
        ...
        self.classes_console_textbox = customtkinter.CTkTextbox(self.consoles_tabview.tab("Classes"),
                                                                ...
                                                                border_width=2,
                                                                font=("consolas", 15), wrap='none')
        self.classes_console_textbox.grid(row=1, column=0, padx=(0, 0), pady=(0, 0), sticky="nsew")
        self.classes_entry = customtkinter.CTkEntry(self.consoles_tabview.tab("Classes"),
                                                          placeholder_text="Search for class name, Press Enter to search",
                                                          font=("consolas", 15))

        self.classes_entry.grid(row=0, column=0, columnspan=100, padx=(0, 0), pady=10, sticky="nsew")
        self.classes_entry.bind("<KeyPress>", self.classes_serach_key_pressed)
        ...
        self.consoles_tabview.tab("Classes").grid_rowconfigure(0, weight=0)
        self.consoles_tabview.tab("Classes").grid_rowconfigure(1, weight=1)
        ...
        self.print_to_console_textBox(text="Current activity automatically updates at the title.", color='green')
        ...
        self.ipc_textbox.tag_config('red', foreground='#FF0000')
        self.ipc_textbox.tag_config('green', foreground='#2AFF00')
        self.ipc_textbox.tag_config('orange', foreground='#FF9700')
        ...
        self.fuzz_exported_activities_button = customtkinter.CTkButton(self.analyzeTabview.tab("Dynamic"), text="Fuzz Exported Activities",
                                                      command=self.fuzz_exported_activities_button_clicked)
        self.fuzz_exported_activities_button.grid(row=0, column=0, padx=75, pady=(5, 5))

        self.stop_fuzz_activities_button = customtkinter.CTkButton(self.analyzeTabview.tab("Dynamic"),
                                                                       text="Stop Fuzzing",fg_color='red',
                                                                       command=self.stop_fuzz_activities_clicked)
        self.stop_fuzz_activities_button.grid(row=1, column=0, padx=75, pady=(5, 5))
    ...
    def classes_serach_key_pressed(self,event):
        if event.keysym == "Return":
            # if event.char.isalpha() or event.char in "!@#$%^&*()._+{}|:\"<>?":
            self.classes_console_textbox.configure(state="normal")
            self.classes_console_textbox.delete('1.0','end')
            self.classes_console_textbox.configure(state="disabled")
            search_class_thread = threading.Thread(target=self.frap.search_class)
            search_class_thread.start()
    def upload_file_explorer(self):
        upload_thread = threading.Thread(target=self.frap.upload_file)
        upload_thread.start()

    def create_folder_file_explorer(self):
        create_folder_thread = threading.Thread(target=self.frap.create_folder)
    ...
    def open_activity_button_clicked(self):
    ...
    def stop_fuzz_activities_clicked(self):
        self.stop_fuzz_activities = True
    def set_frida_app(self, frida_app_instance):
        self.frap = frida_app_instance

    def sign_apk_button_clicked(self):
        self.print_to_console_textBox(
            text=f"[WARNNING] Java (JDK8) has to be installed on your PC ans also be in PATH.", color='orange')
    ...
    def show_current_activity_button_clicked(self):
        ...
        try:
            ...
            self.print_to_console_textBox(text=f"Error in show_current_activity_button_clicked: {traceback.format_exc()}", color='red')
    ...
    def fuzz_exported_activities_button_clicked(self):
        self.consoles_tabview.set("Exploit")
        fuzz_thread = threading.Thread(target=self.frap.fuzz_exported_activities)
        fuzz_thread.start()
    ...
    def open_apk_button_clicked(self):
        try:
            ...
            self.print_to_console_textBox(text=f"Error: {traceback.format_exc()}", color='red')
    ...
    def add_finding_to_finding_tab(self, name, check_var=None, Cvar=None):
        if name not in self.findings:
            ...
            finding_checkbox = customtkinter.CTkCheckBox(self.scrollable_findings, text=name, variable=Cvar, onvalue=1,
                                                         offvalue=0)
    ...
    def print_to_ipc_textBox(self, text, color=''):
        self.ipc_textbox.configure(state="normal")
        if color != '':
            self.ipc_textbox.insert("end", text + "\n", color)
        else:
            self.ipc_textbox.insert("end", text + "\n")
        self.ipc_textbox.configure(state="disabled")
        self.ipc_textbox.yview(tkinter.END)
    ...
    def load_default_bypasses_scripts(self):
        # WITH NO SUFFIX OF .JS!!!!!!!!!!!!!!!!!!!!!
        script_to_auto_toggle = ['Android_Log_class_Watcher', 'Android_file_guard',
                                 'Android_discover_exported_components','Android_search_classes',
                                 'Android_startActivity_listener','Android_deepLink_observer']
    ...
    def sidebar_button_clear_console_clicked(self):
        ...
        if selected_tab == "Console":
            ...
            self.files_console_textbox.configure(state="disabled")
        elif selected_tab == "IPC Traffic":
            self.ipc_textbox.configure(state="normal")
            self.ipc_textbox.delete("2.0", "end")  # Clear the entire content
            self.ipc_textbox.insert("end", "\n")
            self.ipc_textbox.configure(state="disabled")
    ...
    def sidebar_button_save_project_clicked(self):
        try:
            ...
            self.print_to_console_textBox(text=f"Error while saving project: {traceback.format_exc()}", color='red')
    ...
    def sidebar_button_load_project_clicked(self):
        try:
            ...
            self.print_to_console_textBox(text=f"Error while loading project: {traceback.format_exc()}", color='red')
